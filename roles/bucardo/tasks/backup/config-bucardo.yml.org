...
- name: Add the Source DB to configure Bucardo
  become_user: postgres
  shell: |
    bucardo add db {{ db_server_name }} dbhost={{ _ipaddr }} dbport=5432 dbname=testdb dbuser=bucardo dbpass=changeme
...
- name: Add the tables to migrate with table names
  become_user: postgres
  shell: |
...
- name: Exclude one table or few tables as remove table
  become_user: postgres
  shell: |
    bucardo remove table public.table_locks --herd=sampleherd
...
- name: Sequences also to be synced
  become_user: postgres
  shell: |
    bucardo add all sequences --herd=sampleherd
...
- name: Build a Sync
  become_user: postgres
  shell: |
    bucardo add sync samplesync herd=sampleherd dbs=server1:source,server2:target,server3:target,server4:target,server5:target onetimecopy=1
  register: build_sync
  args:
    executable: /bin/bash
    chdir: /home/postgres
